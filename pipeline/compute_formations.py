...
def compute_formation_data(formations: pd.Dataframe, correspondances: pd.Dataframe):
    ...
    try:
        ...
        insert_rome(df_db_rome, db)
        insert_rome_rncp(df_db_rome_rncp, db)
        insert_formation(df_db_formation, db)
    finally:
        db.close()
...
def compute_regions_information():
    ...
    try:
        ...
        insert_region(df_db_region, db)
        insert_department(df_db_department, db)
    finally:
        db.close()
