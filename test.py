...
def stocker_offre_et_competences(cursor, offre):
    """Insere une offre unique, ses competences et met a jour la table pivot."""
    
    # Recuperation de l'ID unique textuel officiel de France Travail
    id_officiel_offre = offre.get("id")
    if not id_officiel_offre:
        return  # Securite : si l'offre n'a pas d'identifiant, on l'ignore
    ...
    # Insertion avec clause ON CONFLICT pour eviter les doublons physiques
    insert_offre_query = """
    ...
    for comp in liste_competences:
        ...
        # Liaison dans la table pivot (ID_Scraping = 0 pour l'API)
        insert_pivot_query = """
        INSERT INTO Offre_Competence (ID_Competence, ID_FranceTravail, ID_Scraping)
        VALUES (%s, %s, 0)
        ON CONFLICT DO NOTHING;
        """
        cursor.execute(insert_pivot_query, (id_competence, id_officiel_offre))
